...
---------------------
Implements an inverted-index search engine for collections of text files.
"""
...
def create_index(filenames, index, file_titles):
    ...
    for filename in filenames:
        with open(filename, encoding='utf-8') as file:
            # The first line is the title.  It is also part of the searchable text.
            ...
            for line in lines:
                for raw_word in line.split():
                    ...
                    # then make the term lowercase.
                    term = raw_word.strip(string.punctuation).lower()

                    # Ignore words made entirely of punctuation.
                    if term == '':
                        continue
                    ...
                    # A file name must occur at most once for each term.
...
def search(index, query):
    ...
    results = list(index[first_term])

    for term in terms[1:]:
        if term not in index:
            return []

        results = common(results, index[term])
